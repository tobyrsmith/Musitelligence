                            ...
                            <v-container v-bind="{ [`grid-list-${'md'}`]: true }" fluid>
                                <v-layout row wrap>
                                    <v-flex v-for="(note,i) in notes_picked" :key="note.id" xs4>
                                        <v-btn fab class="general-btn" color="success" @click="choose(i)">{{note}}
                                        </v-btn>
                                    </v-flex>
                                </v-layout>
                            </v-container>
        ...
        methods: {
            play () {
                if (!this.choosing) {
                    ...
                    this.chose = false
                    this.piano.note(this.notes_picked[this.rand_note] + 3 + 'q').play()
                    this.choosing = true
                    this.questions++
                }
                else {
            ...
            choose (i) {
                if (!this.choosing) {
                    this.listen(i)
                } else if (this.notes_picked[this.rand_note] == this.notes_picked[i]) {
                    this.rightAns()
                } else {
                    this.wrongAns()
                }
            ...
            listen (i) {
                this.piano.note(this.notes_picked[i] + 3 + 'q').play()
            },
            rightAns () {
                if (!this.chose) {
                    this.right_answers++
