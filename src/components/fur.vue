    ...
    <v-app id="inspire">
        ...
        <v-container fluid grid-list-lg>
            ...
            <v-layout row align-center>
                <v-flex xs12>
                    <v-slider name="bpm" label="BPM" :min="40" :max="300" step="5" v-model="bpm" @change="updateBPM"
                              thumb-label="always" class="slider" height="30"></v-slider>
                </v-flex>
            </v-layout>
            <v-flex xs12>
                <v-slider name="volume" prepend-icon="volume_up" min="0" max="1" step="0.01" v-model="vol"
                          @change="updateVol" class="slider" height="30"></v-slider>
            </v-flex>
            <v-layout column wrap align-center>
                <v-flex xs12>
                    <v-btn class="general-btn" color="primary" fab @click="piece.rhythm.toggleMetronome">Toggle
                                                                                                         Metronome
                    </v-btn>
                </v-flex>
                <v-flex xs12>
