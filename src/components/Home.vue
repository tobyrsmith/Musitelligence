        ...
        <template v-if="!composer_on">
            <v-container grid-list-xs>
                ...
                <v-layout wrap justify-center v-if="generated" align-center>
                    ...
                    <v-flex xs1>
                        <v-btn class="general-btn" color="blue" flat icon @click="note.play()"
                        ...
                        <v-layout row justify-center>
                            ...
                            <v-flex x7>
                                <v-layout column wrap>
                                    <v-btn class="general-btn scale_btn" round color="secondary orange" @click="g(s)">
                                        {{note.note}}
                                        {{diatonic[start_index
                                        +
                                        i]['Scale'].length
                                        > 12 ?
                                        diatonic[start_index
                            ...
                            <v-flex xs2>
                                <v-btn class="general-btn" color="blue" flat icon @click="s.play()"
                                       style="background-color: yellow;">
                                    <v-icon>music_note</v-icon>
                                </v-btn>
                            </v-flex>
                            <v-flex xs2>
                                <v-btn class="general-btn" color="blue" flat icon @click="goToComposer(s)"
                                       style="background-color: yellow;">
                                    <v-icon>edit</v-icon>
                                </v-btn>
                            </v-flex>
                                ...
                                <v-flex xs10>
                                    ...
                                    <span v-for="(c, j) in s.chords" :key="c.toString()" class="chords">
                                        <v-layout row wrap justify-center>
                                            ...
                                            <v-flex xs2>
                                                <v-btn class="general-btn" color="blue" flat icon @click="c.play()"
                                                       style="background-color: yellow;">
                                                    <v-icon>music_note</v-icon>
                                                </v-btn>
                                            </v-flex>
                                            <v-flex xs2>
                                                <v-btn class="general-btn" color="blue" flat icon
                                                       @click="c.playMelody()" style="background-color: yellow;">
                                                    <v-icon>music_note</v-icon>
                                                </v-btn>
                                            </v-flex>
                                        </v-layout>
...
<script>
    import {
        DiatonicScale, Note, major_scale, notes, all_scales,
        all_diatonic_scales,
    }                 from 'note-art'
    ...
    export default {
        ...
        data () {
            ...
            return {
                ...
                start_index:    0,
                end_index:      30,
                pages:          Math.ceil(all_diatonic_scales.length / maxScalesShown),
                max_pages:      10,
            }
        },
        components: {
        ...
        methods:    {
            ...
            goToPage (p) {
                this.start_index = 0 + (p - 1) * this.maxScalesShown
                this.end_index   = this.start_index + this.maxScalesShown < this.diatonic.length ? this.start_index + this.maxScalesShown : this.diatonic.length
                this.getPage()
            },
        },
    }
